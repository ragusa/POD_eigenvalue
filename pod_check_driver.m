...
%%
snapshot_file='10snapshots_03_03_2019.mat';
fails=cell(2,1);
for i=1:10
...
%%
snapshot_file='20snapshots_03_03_2019.mat';
fails=cell(2,1);
for i=1:20
    failed=pod_test_anucen(snapshot_file,i);
    if failed(1)>0, fails{1}=[fails{1} failed(1)];end
...
%%
snapshot_file='50snapshots_03_03_2019.mat';
fails=cell(2,1);
for i=1:50
    failed=pod_test_anucen(snapshot_file,i);
    if failed(1)>0, fails{1}=[fails{1} failed(1)];end
...
%%
snapshot_file='100snapshots_03_03_2019.mat';
fails=cell(2,1);
for i=1:100
    failed=pod_test_anucen(snapshot_file,i);
    if failed(1)>0, fails{1}=[fails{1} failed(1)];end
